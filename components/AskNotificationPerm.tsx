...
export function AskNotificationPerm() {
	const { isDenied, isGranted, isSubscribed, isSupported, handleSubscribe } = useNotification()
	...
	useEffect(() => {
		...
		toast({
			...
			description: "Be in-the-loop with our latest updates by enabling notifications.",
			action: (
				<Button variant="outline" size="sm" className="gap-1" onClick={handleSubscribe}> Subscribe </Button>
			)
		})
